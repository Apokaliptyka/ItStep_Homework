...
class NumberSetProxy(FileNumberSet):
    ...
    def __init__(self, real_subject:FileNumberSet, log_filename):
    ...
    def get_numbers(self):
        if os.path.exists(self.real_subject.filename):
            ...
            self.log(summa)
            return self.real_subject.get_numbers()
        else:
    ...
    def update_file(self,new_number):
        if os.path.exists(self.real_subject.filename):
            summa=self.real_subject.get_summa()
            with open(self.real_subject.filename, 'a') as file:
                file.write(new_number + "\n")
            self.log(summa)
...
if __name__ == '__main__':
    ...
    proxy=NumberSetProxy(res,log_life)

    proxy.update_file("488444")
    # proxy.update_file("1266666")
